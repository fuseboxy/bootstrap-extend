...
function ($) {
    ...
    $.fn.blockUI = function (action, options) {
        ...
        return this.each(function () {
            ...
            let storedOptions = $target.data('blockui-options') || {};

            // Save/update options if provided
            if (options) {
                storedOptions = $.extend({}, defaults, storedOptions, options);
                $target.data('blockui-options', storedOptions);
            }

            if (action === 'hide') {

                $target.children('.jq-blockui-overlay').remove();

                const originalPosition = $target.data('blockui-original-position');

                if (originalPosition !== undefined) {
                    $target.css('position', originalPosition);
                    $target.removeData('blockui-original-position');
                }
            ...
            if (originalPosition === 'static') {
                $target.data('blockui-original-position', originalPosition);
                $target.css('position', 'relative');
            }

            const overlayZIndex = getOverlayZIndex($target);

            const $overlay = $(`
                <div class="jq-blockui-overlay ${storedOptions.class}">
                    <div class="jq-blockui-content text-center">
                        ${storedOptions.icon
                            ? `<i class="${storedOptions.icon}"></i>`
                            : ''}
                        ${storedOptions.message
                            ? `<div class="mt-2">${storedOptions.message}</div>`
                            : ''}
                    </div>
                </div>
            `);
            ...
            if (storedOptions.style) {
                $overlay.attr(
                    'style',
                    $overlay.attr('style') + ';' + storedOptions.style
                );
            }

            // Prevent all interaction including scroll/touch scroll
            $overlay.on(
                'click mousedown mouseup mousemove dblclick contextmenu wheel ' +
